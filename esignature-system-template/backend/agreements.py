"""
E-Signature Agreement System - Backend API
This module handles all agreement-related operations
"""
...
# Assuming you have these from your main app
...
class Agreement(Base):
    ...
    campaign_id = Column(String, nullable=False)  # Or your entity reference
    contact_id = Column(String, nullable=False)   # Recipient reference
    contact_name = Column(String, nullable=False)
    contact_email = Column(String, nullable=False)
    company = Column(String)
    start_date = Column(String, nullable=False)
    setup_fee = Column(Float, nullable=False)     # Your custom fields
    monthly_fee = Column(Float, nullable=False)   # Your custom fields
    campaign_name = Column(String, nullable=False) # Or your entity name
...
class SendAgreementsRequest(BaseModel):
...
async def send_agreements(
    request: SendAgreementsRequest,
    current_user: User = Depends(auth.get_current_active_user),
    db: Session = Depends(get_db)
):
    """Send agreements to multiple recipients"""
    sent_count = 0
    failed_count = 0
    ...
    for contact_id in request.contact_ids:
        try:
            ...
            # Create agreement
            agreement = Agreement(
                campaign_id=request.agreement_data.get('campaign_id'),
                contact_id=contact.id,
                contact_name=f"{contact.first_name} {contact.last_name}",
                contact_email=contact.email,
                company=contact.company,
                start_date=request.agreement_data['start_date'],
                setup_fee=float(request.agreement_data['setup_fee']),
                monthly_fee=float(request.agreement_data['monthly_fee']),
                campaign_name=request.agreement_data.get('campaign_name', ''),
                status='pending'
            )
            ...
            # Generate URL
            base_url = get_base_url()  # Your method to get the base URL
            agreement_url = f"{base_url}/agreement/{agreement.id}"
            agreement.agreement_url = agreement_url
            
            # Commit immediately so agreement is accessible
            db.commit()
            
            # Optional: Send email
            # send_agreement_email(contact.email, agreement_url)
            ...
            print(f"Error processing agreement for contact {contact_id}: {e}")
            db.rollback()
            failed_count += 1
...
def generate_agreement_pdf(agreement: Agreement) -> io.BytesIO:
    ...
    terms = """
    1. <b>Services:</b> Provider agrees to provide services as outlined.<br/>
